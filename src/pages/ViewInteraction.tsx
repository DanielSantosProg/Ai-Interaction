...
import { Building, Building2, Calendar, CalendarPlus, GalleryVerticalEnd, ListFilter, Wallet } from "lucide-react"
...
const ViewInteraction = ({ isSidebarOpen }: ViewInteractionProps) => {
    ...
    const {titulo, dataCriacao, solicitante, prompt, dataInicio, dataFim,
        empresa, estabelecimento, localizacao
    } = location.state;
    ...
    <div className="flex flex-row h-screen">
        ...
        <div className={`flex flex-col flex-grow items-center py-12 overflow-y-auto scrollbar-thin`}>
            <div className="flex flex-col items-center">
                <h2 className="text-[26px] sm:text-[32px] pb-[12px] text-center">{titulo}</h2>
                <div className="flex flex-row items-center">
                    <CalendarPlus className="text-[#323232]" size={18}/>
                    <p className="px-2">{dataCriacao}</p>
                </div>
            </div>
            <div className="flex flex-col w-full lg:px-17 max-w-4xl items-center my-12">
                ...
                <div className="flex flex-col lg:self-start gap-1">
                    <div className="flex flex-row items-center gap-2 mb-4 ml-7 lg:self-start">
                        <Calendar className="text-[#323232]" size={16}/>
                        <p className="text-[#323232]">{dataInicio} - {dataFim}</p>
                    </div>
                    <div className="flex flex-row items-center gap-2 mb-4 ml-7 lg:self-start">
                        <Building2 className="text-[#323232]" size={16}/>
                        <p className="text-[#323232]">{empresa}</p>
                    </div>
                    <div className="flex flex-row items-center gap-2 mb-4 ml-7 lg:self-start">
                        <Building className="text-[#323232]" size={16}/>
                        <p className="text-[#323232]">{estabelecimento}</p>
                    </div>
                    <div className="flex flex-row items-center gap-2 mb-4 ml-7 lg:self-start">
                        <Wallet className="text-[#323232]" size={16}/>
                        <p className="text-[#323232]">{localizacao}</p>
                    </div>
