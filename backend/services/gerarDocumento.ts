    ...
    try {
        ...
        if (modelo === "modelo1") {
            ...
            if (dataInicial) {
                ...
                request.input('dataInicial', mssql.VarChar(20), dataInicial);
            }
            if (dataFinal) {
                stringFilters += ` AND COR_DUP_DATA_VENCIMENTO <= @dataFinal`;
                request.input('dataFinal', mssql.VarChar(20), dataFinal);
            }
            if (empId) {
...
async function gerarDocumento(dir: string, titulo: string, modelo: string, sqlPool: any, dataInicial?: string, dataFinal?: string, empId?: number, estabelecimentoId?: number, localizacaoId?: number) {
    try {
